...
async def get_ai_answer(user_question: str, context: str) -> str:
    ...
    system_prompt = (
        "Ты помощник студента КБТУ. Старайся отвечать развернуто и по факту Отвечай ТОЛЬКО на основе предоставленного ниже контекста." 
        "Если в контексте НЕТ прямого ответа на вопрос, ЗАПРЕЩЕНО выдумывать или комбинировать не связанные факты." 
        "В таком случае ответь одним словом: NO_INFO"
    )
